...
public class RestController extends SCController {
	...
	// ---- Mongo operations ----
	/**
	 * Method createUri: used to create the uri of the query operation to 'sleepy mongoose' rest server.
	 * @param host
	 * @param port
	 * @param db
	 * @param collection
	 * @return
	 */
	protected String createUri(String host, String port, String db, String collection){
		return String.format("http://%s:%s/%s/%s/", host, port, db, collection);
	}
	
	/**
	 * Method composeQuery: used to compose a 'sleepy mongoose' query with the parameters passed in input.
	 * @param params: list of string with the parameters to consider in the query;
	 * @param values: list of string wiht parameter's values to consider in the query;
	 * @param types: list of types of the values passed in input
	 * @param condition: condition to be used when the parameters are aggregated ('and' or 'or')
	 * @return
	 */
	protected String composeQuery(List<String> params, List<String> values, List<Integer> types, int condition){
		String query = "";
	...
	/**
	 * Method composeSingleParam: used to create the query string when using 'sleepy mongoose' server.
	 * @param param String with the param to add in the query;
	 * @param value String with the param's value to consider in the query
	 * @param type int: type of the value: if 0, object or int; if 1 string; 
	 * if 2 condition grater than; if 3 condition little than.
